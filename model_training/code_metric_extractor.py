...
def extract_metrics_from_zip(zip_path: str, extract_to: str = None, overwrite: bool = True) -> list:
    """
    Extract ZIP and return metrics for every source file inside.
    Returns: list of {'file': name, 'metrics': {...}, 'is_log': bool, 'log_bugs': [...]}

    overwrite=False skips re-extracting when extract_to already has content —
    used when re-running analysis on a working copy that may hold edits made
    since the original extraction (in-app editor or an external editor).
    """
    ...
    results = []
    if overwrite or not os.listdir(extract_to):
